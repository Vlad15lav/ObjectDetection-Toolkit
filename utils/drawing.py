...
def draw_sample(img_path, bboxes, labels, scale=1, xyxy=True):
...
def draw_batch(imgs_paths, bboxes, labels, scale=1, xyxy=True):
    for path, bbox, label in zip(imgs_paths, bboxes, labels):
        draw_sample(path, bbox, label, scale=scale, xyxy=xyxy)
